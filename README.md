# Pame — turn your Windows PC into a console

<p align="center"><img src="src/Pame.App/Assets/Brand/wordmark.png" alt="Pame" width="480"></p>

A native Windows gaming shell built around a controller. Pame brings installed games into a fullscreen library, opens their stores when needed, tracks local play sessions and provides a system quick menu.

**[Download Pame for Windows x64](https://github.com/LielZ/Pame/releases/download/v0.4.1/Pame-Setup-0.4.1-x64.exe)** · **[Releases](https://github.com/LielZ/Pame/releases)** · **[Report a problem](https://github.com/LielZ/Pame/issues)**

**Version 0.4.1 — working alpha.** Includes an embedded browser, Quick menu media player, controller status overlays, hold-to-drag mouse control, navigation audio, game profiles and hardware monitoring. See [CHANGELOG.md](CHANGELOG.md) and [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md).

## Why Pame?

PC gaming on a TV often means switching launchers, hunting for a mouse, dismissing desktop windows and finding your way back after a game closes. Pame puts that routine behind a controller-friendly interface.

- **One place to play:** browse supported installed libraries, favorites and recent games without opening every launcher yourself.
- **A console-style transition:** hide the taskbar and desktop icons, minimize open windows before launching, show the game's artwork as the desktop background, and return to Pame when the game ends.
- **Keep the controller in your hands:** navigate menus, type with an onscreen keyboard, and use the sticks as a Windows or browser mouse when a store needs your attention.
- **Media beside your games:** use tabs and favorites in Pame browser, open it from the in-game Quick menu, and control supported media sources with the right stick.
- **Choose what runs during play:** select background apps and optional services to pause/close or reduce; Pame restores its changes afterwards and keeps reopened app windows hidden.

Pame runs on top of Windows and uses your existing stores and games. Store accounts, purchases, updates, DRM and anti-cheat remain with their providers. No games are bundled.

**בעברית:** Pame הופכת את חוויית השימוש במחשב גיימינג לחוויה של קונסולה: ספריית משחקים אחת, ניווט בשלט, דפדפן ומדיה בתוך התוכנה, מעבר נוח למשחק וחזרה אוטומטית לממשק. היא נועדה לצמצם את הצורך בעכבר ובמקלדת בזמן משחק מהספה.
...
Look & sound also selects locally installed sound packs. This PC has a community PS5 recording pack under `%LOCALAPPDATA%\Pame\sounds\PlayStation`; it is preserved during upgrades. The installer ships the original Pame sounds. See `docs/PLAYSTATION_SOUND_SOURCE.json` for local-pack provenance.

## Validation

See [docs/VALIDATION-0.4.1.md](docs/VALIDATION-0.4.1.md) for this update, [docs/VALIDATION-0.4.0.md](docs/VALIDATION-0.4.0.md) for the browser/controller update, [docs/VALIDATION-0.3.3.md](docs/VALIDATION-0.3.3.md) for background app/service cycles and [docs/VALIDATION.md](docs/VALIDATION.md) for the original baseline. Native integration probes exercise media controls, window recovery, pointer input, embedded navigation, notifications and game transitions. The prior Fortnite wallpaper/return fix was confirmed by the user; broader game and hardware coverage remains ongoing.
